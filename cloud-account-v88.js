// START/NOW v88 — optional accounts, cloud backup/sync, account deletion, and production identity polish.
(() => {
  ...
  function applyIdentity(){
    const name = displayName();
    document.querySelectorAll(".avatar, .profile-avatar").forEach(node => { node.textContent = initials(); });
    const profileHeading = document.querySelector(".profile-card h2");
    if (profileHeading) profileHeading.textContent = name || "Your profile";
  }
  ...
  if (appRoot) new MutationObserver(() => { applyIdentity(); if (typeof state !== "undefined" && state.page === "profile") appendAccountCard(); }).observe(appRoot,{childList:true,subtree:true});
  ...
  window.START_NOW_CLOUD = {version:"v88",syncNow:() => syncNow(),openSignIn:() => openAuth("signin"),openSignUp:() => openAuth("signup")};
