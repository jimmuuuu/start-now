...
(() => {
  ...
  if (params.get("personal") !== "arnold-v20") return;
  ...
  const ex = {
    ...
    frontRaise: () => findExercise(["Dumbbell Front Raise", "Cable Front Raise"], "Shoulders", 2, 12),
  ...
  const split = [
    { name: "Chest & Back A", day: "Monday", exercises: [ex.bench(), ex.inclineBar(), ex.pullup(), ex.row(), ex.fly(), ex.pullover()] },
    { name: "Shoulders & Arms A", day: "Tuesday", exercises: [ex.arnoldPress(), ex.lateral(), ex.rearDelt(), ex.barCurl(), ex.inclineCurl(), ex.overheadTri(), ex.pushdown()] },
    { name: "Legs A", day: "Wednesday", exercises: [ex.squat(), ex.legPress(), ex.rdl(), ex.legCurl(), ex.legExt(), ex.calf()] },
    { name: "Chest & Back B", day: "Thursday", exercises: [ex.inclineDb(), ex.dbBench(), ex.pullup(), ex.tbar(), ex.fly(), ex.pullover()] },
    { name: "Shoulders & Arms B", day: "Friday", exercises: [ex.overhead(), ex.frontRaise(), ex.cableLateral(), ex.rearDelt(), ex.preacher(), ex.hammer(), ex.skull(), ex.pushdown()] },
    { name: "Legs B", day: "Saturday", exercises: [ex.frontSquat(), ex.hack(), ex.rdl(), ex.legCurl(), ex.lunge(), ex.seatedCalf()] }
  ];

  // Remove any previous personal Arnold install, but keep all other workouts in the library.
  // Unscheduled workouts remain available; only this split controls the weekly schedule.
  ...
  state.customWorkouts.push(...arnoldWorkouts);
  saveCustomWorkouts();
  localStorage.setItem("sn_personal_plan", "arnold-v20");
  localStorage.setItem("sn_personal_arnold_v20", "applied");
  localStorage.removeItem("sn_personal_arnold_v19");

  // Run after all page wrappers are installed so the Workouts tab immediately reflects the change.
  state.page = "workouts";
  render();
  if (typeof showToast === "function") showToast("Your Arnold split is now your active schedule");
})();
