...
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .dark .sn66-mode.active{background:#16253B}.dark .sn66-mode-icon,.dark .sn66-existing-icon,.dark .sn66-add{background:#1B2A42}.dark .sn66-order button{background:#24282D}
      @media(max-width:620px){.sn66-modes{grid-template-columns:1fr}.sn66-mode{min-height:0;display:grid;grid-template-columns:44px 1fr;column-gap:12px}.sn66-mode-icon{grid-row:1/3;margin:0}.sn66-tools{grid-template-columns:1fr}.sn66-top h1{font-size:27px}.sn66-preview-actions{grid-template-columns:1fr}}
    `;
  ...
  function diversifiedPick(candidates, count, focus) {
    ...
    const add = ex => {
      ...
      usedIds.add(id); selected.push(ex);
    };
  ...
  function renderQuickWorkout() {
    ...
    if (qw.mode === 'existing') {
      document.querySelectorAll('[data-existing]').forEach(btn=>btn.addEventListener('click',()=>{ const workout=(state.customWorkouts||[]).find(w=>String(w.id)===btn.dataset.existing); if(workout) startQuick(workout,'existing'); }));
    }

    if (qw.mode === 'surprise') {
      document.querySelectorAll('[data-surprise-key]').forEach(btn=>btn.addEventListener('click',()=>{ qw.surprise[btn.dataset.surpriseKey]=btn.dataset.surpriseValue; qw.surprise.generated=null; qw.surprise.actualFocus=null; renderQuickWorkout(); }));
      const generate = () => { const picked=generatedWorkout(); renderQuickWorkout(); if(!picked.length){ const empty=document.getElementById('sn66SurpriseEmpty'); if(empty) empty.textContent='There are not enough verified exercises for those filters yet. Try Full gym or a broader training focus.'; } };
      ...
      document.getElementById('sn66StartSurprise')?.addEventListener('click',()=>{ const exercises=(qw.surprise.generated||[]).map(ex=>({...ex})); if(exercises.length) startQuick({name:`${qw.surprise.actualFocus} Quick Workout`,exercises},'surprise'); });
    }
  }
  ...
  function enhanceSummary() {
    ...
    document.getElementById('sn66SaveQuick')?.addEventListener('click',()=>{ if(saveQuickSession(session)){ card.innerHTML='<h3>Saved to My Workouts</h3><p>You can schedule it later from the Workouts tab if you want.</p>'; } });
