// START/NOW schedule editor v14 — reliable pointer-based drag for desktop and mobile.
(() => {
  ...
  function installStyles(){
    if(document.getElementById("snScheduleV14Styles")) return;
    const style = document.createElement("style");
    style.id = "snScheduleV14Styles";
    style.textContent = `
      ...
      .sn-schedule-row.sn-dragging{opacity:.55;transform:scale(.985);box-shadow:0 12px 30px rgba(0,0,0,.18);cursor:grabbing}
  ...
  function resetDrag(row,pointerId){
    if(row && pointerId !== undefined){
  ...
  function bindRowDrag(){
    document.querySelectorAll(".sn-schedule-row").forEach(row => {
      ...
      if(!draft[sourceDay]) return;

      row.setAttribute("aria-label", `${workoutById(draft[sourceDay])?.name || "Workout"} on ${sourceDay}. Click and drag this card to another day.`);
      ...
      row.addEventListener("pointermove", e => {
        if(!dragState || dragState.pointerId !== e.pointerId || dragState.sourceDay !== sourceDay) return;
        ...
        e.preventDefault();
        highlightTarget(sourceDay,rowAtPoint(e.clientX,e.clientY));
      });

      row.addEventListener("pointerup", e => {
        if(!dragState || dragState.pointerId !== e.pointerId || dragState.sourceDay !== sourceDay) return;
        const wasActive = dragState.active;
        const target = wasActive ? rowAtPoint(e.clientX,e.clientY) : null;
        resetDrag(row,e.pointerId);
        if(wasActive && target) moveWorkout(sourceDay,target.dataset.day);
      });
  ...
  function renderEditor(){
    ...
    modal.innerHTML = `
      ...
      <p class="sn-schedule-sub">Click and hold a workout card, then drag it onto another day.</p>
      <div class="sn-drag-tip"><strong>Drag the workout card itself.</strong> Drop it on another workout to swap them, or on a rest day to move it there. The Change button still works for manual editing.</div>
