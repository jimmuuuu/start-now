...
(() => {
  ...
  function card(ex, compact = false) {
    ...
    return `<section class="sn-v42-card ${compact ? 'compact' : ''}" data-v42-exercise="${esc(MEDIA.canonicalId(ex))}">
      <div class="sn-v42-head"><div><span>EXERCISE GUIDE</span><strong>${esc(ex?.name || 'Exercise')}</strong></div>${result.status === 'ready' ? '<small>VERIFIED MEDIA</small>' : ''}</div>
      ...
      ${compact ? '' : `<div class="sn-v42-workout-meta"><strong>${esc(ex?.name || 'Exercise')}</strong><span>${esc(workoutSummary(ex))}</span></div><div class="sn-v42-cue"><span>COACH CUE</span><p>${esc(ex?.cue || meta(ex).instructions || 'Use a controlled range of motion and keep your setup consistent.')}</p></div>`}
    </section>`;
  }
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .sn-v42-primary{display:flex;gap:8px;align-items:baseline;padding:12px 16px 10px}.sn-v42-primary span,.sn-v42-cue span{font-size:9px;font-weight:900;letter-spacing:.09em;color:var(--muted)}.sn-v42-primary strong{font-size:12px;color:var(--text)}
      ...
      .sn-v42-cue{padding:12px 16px 15px;border-top:1px solid var(--line)}.sn-v42-cue p{margin:5px 0 0;font-size:12px;line-height:1.5;color:var(--muted)}
      ...
      .sn-v42-card.compact{margin:10px 0 16px;box-shadow:none}.sn-v42-card.compact .sn-v42-head{padding:12px 14px 9px}.sn-v42-card.compact .sn-v42-workout-meta,.sn-v42-card.compact .sn-v42-cue{display:none}.sn-v42-card.compact .sn-v42-media,.sn-v42-card.compact .sn-v42-demo-pair,.sn-v42-card.compact .sn-v42-frame-stack,.sn-v42-card.compact .sn-v42-unavailable{min-height:165px}
