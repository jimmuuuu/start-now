// START/NOW v70 — clean Quick Actions routing; legacy Quick Workout UI removed.
(() => {
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .sn70-quick-actions .tiles{margin:0}.sn70-quick-actions .tile{cursor:pointer;transition:transform .16s ease,box-shadow .16s ease}.sn70-quick-actions .tile:hover{transform:translateY(-2px);box-shadow:0 12px 24px rgba(20,25,35,.10)}.sn70-quick-actions .tile:active{transform:scale(.985)}
  ...
  function enhanceHome(){
    ...
    if(!section){section=document.createElement('section');section.className='sn70-quick-actions';oldTiles.replaceWith(section);}
    section.innerHTML=`<div class="sn70-head"><strong>Quick Actions</strong></div><div class="tiles">${actions.map(item=>`<button class="tile ${item.tone}" data-sn70-action="${item.id}" aria-label="${esc(item.title)} — ${esc(item.subtitle)}"><div class="sn70-icon">${icon(item.icon,28)}</div><strong>${esc(item.title)}</strong><span>${esc(item.subtitle)}</span></button>`).join('')}</div>`;
    section.querySelectorAll('[data-sn70-action]').forEach(btn=>btn.addEventListener('click',()=>openAction(btn.dataset.sn70Action)));
  }

  function openAction(id){
    if(id==='quickWorkout'){
      state.page='quickWorkout';
      if(window.START_NOW_QUICK_WORKOUT?.render) window.START_NOW_QUICK_WORKOUT.render();
      else render();
      return;
    }
  ...
  window.render=function(...args){
    if(state.page==='quickWorkout' && window.START_NOW_QUICK_WORKOUT?.render){
  ...
  // Exercise Library returns to Home when launched from Quick Actions.
  document.addEventListener('click',event=>{
    if(state.page!=='exerciseLibrary'||!state.__quickActionLibraryReturn) return;
    const back=event.target.closest?.('#snBack');
    if(!back) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    state.page=state.__quickActionLibraryReturn;
    state.__quickActionLibraryReturn=null;
    render();
  },true);

  // Bottom center + now opens the same modern Quick Workout screen.
  ...
  if(state.page==='home') enhanceHome();
  window.START_NOW_QUICK_ACTIONS={version:'v70',actions,renderStats};
})();
